...
::: {.activity-meta}
**Objetivo:** estimar `y = a + bx` usando a forma matricial `(XTX)1XTy`.  
**Dados:** `dados_x.txt` (anos de estudo) e `dados_y.txt` (salario mensal).  
**Ferramentas:** `numpy`, `pandas`, `plotly.express`.
:::
...
ordem = np.argsort(x)
# Grafico segue o template solicitado usando plotly.express.scatter
fig_regressao = px.scatter(
    x=x,
    y=y,
    title="Relacao entre anos de estudo e salario (regressao linear)",
    labels={"x": "Anos de estudo", "y": "Salario observado (R$)"},
)
fig_regressao.update_traces(
    name="Observado",
